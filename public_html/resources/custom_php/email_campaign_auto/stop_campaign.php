<?php
// stop_campaign.php - Stop an email campaign
...
$version = '2025-01-25-Dynamic-Paths';
log_message("Starting stop_campaign.php (Version: {$version})");
...
log_message("Attempting to stop campaign {$campaign_id}");
...
if ($progress['status'] === 'stopped') {
    log_message("Campaign {$campaign_id} is already stopped");
    echo json_encode(['success' => false, 'error' => 'Campaign is already stopped']);
    exit;
}
...
$progress['messages'][] = "Campaign stopped at " . date('Y-m-d H:i:s');
...
for ($attempt = 1; $attempt <= 3; $attempt++) {
    ...
    if ($result !== false) {
        ...
        $write_success = true;
        log_message("Successfully stopped campaign {$campaign_id}, wrote to {$progress_file} on attempt {$attempt}");
        break;
    }
...
if (!$write_success) {
    log_message("Failed to stop campaign {$campaign_id}: Unable to write to {$progress_file} after 3 attempts");
    echo json_encode(['success' => false, 'error' => 'Failed to stop campaign']);
    exit;
}
